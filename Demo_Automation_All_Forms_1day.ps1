...
#Getting dates
$datefrom = Get-Date -Date $reportEnd.AddDays(-15) -Format "yyyy-MM-ddTHH:mm:ss"

$dateTo= $reportEnd

$projectids = Get-Content C:\scripts\demo_project_id.txt
...
#Export to file
$formsList | Export-Csv -Path c:\scripts\demo_formslist.csv -append -NoTypeInformation

#Exporting only last data, creating new and apending data

$formsList | Export-Csv -Path c:\scripts\demo_formslist_current.csv -append  -NoTypeInformation
...
$cleaned = Import-Csv C:\scripts\demo_formslist.csv| Sort-Object FormID -Unique

Start-Sleep -Seconds 0.5

$cleaned | Sort-Object |  Export-Csv -Path C:\scripts\demo_formslist.csv -NoTypeInformation
...
$SourceFilePath ="c:\scripts\demo_formslist.csv"
$DestinationPath = "Demo" #Site Relative Path of the Library
